...
| `_cloudaz/_engine/` | Shared paginated-endpoint engine for migrated CloudAz services: `RequestPlan` (one HTTP request description), `PageDialect`/`PageShape` (paging query-param vocabulary per response shape-family), `PaginatedSpec` (endpoint spec: model, method, dialect, plan builder), free-function spec constructors (`query_paginated`, `search_paginated`), and `SyncEndpointRunner`/`AsyncEndpointRunner` that interpret a spec against an injected `httpx.Client`/`AsyncClient`. `assemble_page` is the single `PageResult` assembler, dispatching to the existing `_response.py` parsers by shape. Internal (underscore-prefixed); not exported from any public facade. |
...
- Large vendor fixture: `tests/openapi/_openapi/fixtures/nextlabs-openapi.json`
